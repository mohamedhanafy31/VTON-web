...
app.get('/trials', async (req, res) => {
    ...
    try {
        let trialsData;
        const trialsFilePath = `${__dirname}/trials.json`;
        console.log('Reading trials from:', trialsFilePath);
        try {
            const data = await fs.readFile(trialsFilePath, 'utf8');
            trialsData = JSON.parse(data);
            console.log('Trials data read:', trialsData);
        } catch (error) {
            console.warn('Trials file not found or invalid, initializing with default value');
            trialsData = { num_trials: 10 };
            await fs.writeFile(trialsFilePath, JSON.stringify(trialsData, null, 2));
            console.log('Initialized trials.json with:', trialsData);
        }
        res.json(trialsData);
    } catch (error) {
...
app.post('/update-trials', async (req, res) => {
    ...
    try {
        ...
        const trialsFilePath = `${__dirname}/trials.json`;
        console.log('Writing trials to:', trialsFilePath);
        await fs.writeFile(trialsFilePath, JSON.stringify(trialsData, null, 2));
        console.log('Trials updated successfully:', trialsData);
    ...
    try {
        ...
        if (!folder || folder === 'VirtualTryOn_Images') {
            ...
            try {
                ...
                console.warn('Descriptions file not found, initializing empty object');
            }
            ...
            console.log('Updated descriptions.json');
        }
...
app.get('/descriptions', async (req, res) => {
    ...
    try {
        ...
        try {
            const data = await fs.readFile(descriptionsFile, 'utf8');
            descriptions = JSON.parse(data);
            console.log('Descriptions read:', Object.keys(descriptions).length);
        } catch (error) {
            console.warn('Descriptions file not found, returning empty object');
        }
...
app.post('/tryon', async (req, res) => {
    ...
    try {
        ...
        let trialsData;
        try {
            trialsData = JSON.parse(await fs.readFile(`${__dirname}/trials.json`, 'utf8'));
        } catch (error) {
            trialsData = { num_trials: 10 };
            await fs.writeFile(`${__dirname}/trials.json`, JSON.stringify(trialsData, null, 2));
        }
        if (trialsData.num_trials <= 0) {
            console.warn('No trials remaining');
